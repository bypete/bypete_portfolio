...
<Note theme="note" display="block" hasIcon={true} class='my-fluid-m'>
...
<p class="text-step-1">In Autumn 2015, the site, formerly known as kerv.com, debuted as a simple static support site for the Kickstarter crowdfunding campaign. I created the static templates using the Foundation v5 framework, which claimed to be at the cutting edge of responsive development at the time.</p>
...
<Figure x-data="{compare: true}" grid="flex flex-col md:flex-row items-center gap-fluid-m p-fluid-s-m" caption='An example of before (left) and after (right) the spring refresh. The brief was to reduce the visual gap between app and website.  Tap each image to view in detail' credit={frontmatter.title}>
...
<BlockPicture src={block__cyclist_40} class="my-fluid-2xl" alt="Close up of K Ring and cyclist" credit={frontmatter.title} wrapper={true} />
